...
function validateContactDetails($contact) {
    if($contact->getFirstName() == "" || $contact->getLastName() == "" || $contact->getEmail() == "" ||
        $contact->getCategory() == "" || $contact->getDesignation() == "" || $contact->getPhone1() == "" ||$contact->getPhone2() == "" || 
...
function setContactDetails($client_company_id) {
    ...
    for($i = 1; $i <= $clientTotalContacts; $i++) {
        ...
        $Email = $_POST['contact_form_'.$i.'_email'];
        $Email2 = $_POST['contact_form_'.$i.'_email2'];
        $Email3 = $_POST['contact_form_'.$i.'_email3'];
        $email = array();
        ...
        $contact->setPhone1($_POST['contact_form_'.$i.'_phone1']);
        $contact->setPhone2($_POST['contact_form_'.$i.'_phone2']);
...
function setContactDetailsFromCsv($max_client_company_id) {
    ...
    if(!empty($_FILES['contactsCSV']['name']) && in_array($_FILES['contactsCSV']['type'], $csvMimes)) {
        ...
        while(($data = fgetcsv($contactsCSVFile)) !== false) {
            $country = $locationService->getCountryByName($data[10]);
            $state = $locationService->getStateByName($data[9]);
            $city = $locationService->getCityByName($data[8]);
            $contact = new Contact();
            ...
            $contact->setPhone1($data[3]);
            $contact->setPhone2($data[4]);
            $contact->setCategory($data[5]);
            $contact->setDesignation($data[6]);
            $contact->setAddress($data[7]);
            $contact->setLinkedIn($data[11]);
            $contact->setFacebook($data[12]);
            $contact->setTwitter($data[13]);
