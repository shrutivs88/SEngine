    ...
    */
    ...
    if(($_FILES['attach1']['name'] !== "") || isset($_FILES['attach2']['tmp_name']) || isset($_FILES['attach3']['tmp_name']))  {
        ...
        for($x=0;$x<count($files);$x++)
        {
            ...
            $fileSize = filesize($files[$x]['tmp_name']);
            if($fileSize > 0){
            ...
            $message .= "--{$mime_boundary}\n";  
                 ...
                 $_SESSION['errormsg'] = "Attached file size is 0. Please attach a file having size greater than 0";
             $_SESSION['sub'] =  $_POST['sub'];
     ...
     { 
        ...
        $password = $row_mail_config->mail_config_password;                                    
         ...
         $fileNames = array();
           ...
           /*   $sqlstatus = "select email_timeout from client_contacts where  client_contact_id='$id'";
            $resstatus = mysqli_query($conn , $sqlstatus);
            $rowstatus = mysqli_fetch_object($resstatus);
            $stmt_status = "select mail_activity_sent_time,mail_activity_to from mail_activities where mail_activity_to = '$to'";
   
            $stmt_res = mysqli_query($conn,$stmt_status);
        
                    $row_status = mysqli_fetch_object($stmt_res);
                    ...
                    $minute = $since_start->days * 24;
                    $minute += $since_start->h;
                    $minute *=60 ;
                    $minute += $since_start->i; 
            
                    $timeset = $rowstatus->email_timeout * 60;
                    if(($rowstatus->email_timeout == 0) || ($timeset >= $rowstatus->email_timeout)){
                        $status = "Send Proposal";
                        $resupdate2="update client_contacts set client_contact_status='$status' where client_contact_id='$id'";
                        mysqli_query($conn,$resupdate2); 
                    }*/
