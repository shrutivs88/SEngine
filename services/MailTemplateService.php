...
class MailTemplateService {
    ...
    public function deleteTemplateById($mailTemplateId) {
     ...
     public function getMailTemplateBasedOnId($mailTemplateId) {
        $stmt = $this->connection->prepare("select * from mail_templates where mail_template_id = ?");
        $stmt->bind_param("i", $mailTemplateId);
        $stmt->execute();
        $res = $stmt->get_result();
        $row = $res->fetch_assoc();
        ...
        $TemplateArray = array();
        array_push($TemplateArray,$header);
        array_push($TemplateArray,$footer);
        $stmt->close();
