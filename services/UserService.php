...
class UserService {
    ...
    public function getAllLikeNameAndRole($searchQuery, $roleId) {
        $searchQueryLike = "%{$searchQuery}%";
        $stmt = $this->connection->prepare("select * from users where user_name like ? and role_id = ?");
        $stmt->bind_param("si", $searchQueryLike, $roleId);
        $stmt->execute();
        $res = $stmt->get_result();
        $listOfUsers = array();
        while($row = $res->fetch_assoc()) {
    ...
    public function getAllLikeEmailAndRole($searchQuery, $roleId) {
        $searchQueryLike = "%{$searchQuery}%";
        $stmt = $this->connection->prepare("select * from users where user_email like ? and role_id = ?");
        $stmt->bind_param("si", $searchQueryLike, $roleId);
        $stmt->execute();
        $res = $stmt->get_result();
        $listOfUsers = array();
