...
class ClientService {
    ...
    public function updateContact($contact) {
       ...
       // $mobile = $contact->getMobile();
        ...
        $stmt = $this->connection->prepare("update client_contacts set client_contact_first_name = ?, client_contact_last_name = ?, client_contact_email = ?, client_contact_category = ?, client_contact_designation = ?, client_contact_phoneno1 = ?, client_contact_phoneno2 = ?, city_id = ?, state_id = ?, country_id = ?, client_contact_address = ?, client_contact_linkedin = ?, client_contact_facebook = ?, client_contact_twitter = ? where client_contact_id = ?");
        $stmt->bind_param("sssssiiiiissssi", $firstName, $lastName, $email, $category, $designation, $phone1, $phone2, $city, $state, $country, $address, $linkedIn, $facebook, $twitter, $id);
        $stmt->execute();
        $stmt->close();
    }
    ...
    public function saveContact($contact) {
        ...
        $emailTimeout = 24;
        $stmt = $this->connection->prepare("insert into client_contacts (client_contact_first_name, client_contact_last_name, client_contact_email, client_contact_category, client_contact_designation,client_contact_phoneno1,client_contact_phoneno2, city_id, state_id, country_id, client_contact_address, client_contact_linkedin, client_contact_facebook, client_contact_twitter, client_contact_status, client_contact_added, client_company_id, assoc_manager_id, assoc_user_id, email_timeout) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssiiiiissssssiiis", $firstName, $lastName, $email, $category, $designation, $phone1, $phone2,$city, $state, $country, $address, $linkedIn, $facebook, $twitter, $status, $added, $companyId, $assocManager, $assocUser, $emailTimeout);
        $stmt->execute();
        $stmt->close();
    }
    ...
    public function getContactsByOffset($offset) {
        ...
        while($row = $res->fetch_assoc()) {
            ...
            $contact->setPhone1($row['client_contact_phoneno1']);
            $contact->setPhone2($row['client_contact_phoneno2']);
           ...
           $client_email_id = array();
                $client_email_ids = explode(',',$row['client_contact_email']);
                $client_email_id =$client_email_ids;
                $client_email_id = implode("<br>",$client_email_id);
            if($row['client_contact_status'] == "Mail Sent"){
                if(function_exists('date_default_timezone_set')) {
                    date_default_timezone_set("Asia/Kolkata");
                    $php_timestamp_date = date("Y-m-d H:i:s");
                }
                
                $mail_activity = "select * from mail_activities where mail_activity_to='$client_email_ids[0]'";  
            ...
            $row['client_contact_email'] = $client_email_id;
    ...
    public function getContactsByCompanyId($companyId) {
        ...
        while($row = $res->fetch_assoc()) {
            ...
            $contact->setFirstName($row['client_contact_first_name']);
            $contact->setLastName($row['client_contact_last_name']);
            $emails = $row['client_contact_email'];
    ...
    public function getContactById($contactId) {
        ...
        if($row = $res->fetch_assoc()) {
            ...
            $contact->setCategory($row['client_contact_category']);
            $contact->setDesignation($row['client_contact_designation']);
           // $contact->getPhone1($row['client_contact_phoneno1']);
           // $contact->getPhone2($row['client_contact_phoneno2']);
            $contact->setPhone1($row['client_contact_phoneno1']);
            $contact->setPhone2($row['client_contact_phoneno2']);
            $contact->setCountry($row['country_id']);
            $contact->setState($row['state_id']);
    ...
    public function getAllCompaniesLikeName($searchQuery) {
        $searchQueryLike = "%{$searchQuery}%";
        $stmt = $this->connection->prepare("select * from client_companies where client_company_name like ?");
    ...
    public function getAllCompaniesLikeWebsite($searchQuery) {
        $searchQueryLike = "%{$searchQuery}%";
        $stmt = $this->connection->prepare("select * from client_companies where client_company_website like ?");
        $stmt->bind_param("s", $searchQueryLike);
        $stmt->execute();
        $res = $stmt->get_result();
        $listOfCompanies = array();
    ...
    public function getAllContactsLikeFirstName($searchQuery) {
        $searchQueryLike = "%{$searchQuery}%";
        $stmt = $this->connection->prepare("select * from client_contacts where client_contact_first_name like ?");
        $stmt->bind_param("s", $searchQueryLike);
        $stmt->execute();
        $res = $stmt->get_result();
        $listOfContacts = array();
    ...
    public function getAllContactsLikeLastName($searchQuery) {
        $searchQueryLike = "%{$searchQuery}%";
        $stmt = $this->connection->prepare("select * from client_contacts where client_contact_last_name like ?");
        $stmt->bind_param("s", $searchQueryLike);
        $stmt->execute();
        $res = $stmt->get_result();
        $listOfContacts = array();
    ...
    public function getAllContactsLikeEmail($searchQuery) {
        $searchQueryLike = "%{$searchQuery}%";
        $stmt = $this->connection->prepare("select * from client_contacts where client_contact_email like ?");
        $stmt->bind_param("s", $searchQueryLike);
        $stmt->execute();
        $res = $stmt->get_result();
        $listOfContacts = array();
        ...
        $listOfContactsforbde = array();
            ...
            if($row['client_contact_status'] == "Mail Sent")
                {   
                    ...
                    $row["email_timeout"] = $row["email_timeout"] * 60;
                    if(($minutes >= $row["email_timeout"]) || ($row["email_timeout"] == 0))
                        {
                        $status="Send Proposal";
                        ...
                        $stmt_clientUpdate->bind_param("s", $status);
        ...
        if($row['client_contact_status'] == "Mail Sent")
            {   
                ...
                if(($minutes >= $row["email_timeout"]) || ($row["email_timeout"] == 0))
                    ...
                    $stmt_clientUpdate->execute();
                    //$res_clientUpdate = $stmt_clientUpdate->get_result();
                   // $row_clientUpdate = $res_clientUpdate->fetch_object();
                    $row["client_contact_status"] = $status;
                    }
            }
            $contact->setStatus($row['client_contact_status']);
...
public function getCountOfBDEsAndClientCountacts($userid){
$stmt_bdm =  $this->connection->prepare("select user_emp_id from users where user_id = ?");
$stmt_bdm->bind_param("i", $userid);
$stmt_bdm->execute();
$res_bdm = $stmt_bdm->get_result();
$row_bdm = $res_bdm->fetch_assoc();
$bdm_emp_id = $row_bdm['user_emp_id'];
$bdeAndClientCount = array(); 
$stmt = $this->connection->prepare("select * from client_contacts where assoc_manager_id = ?");
$stmt->bind_param("i", $bdm_emp_id);
$stmt->execute();
$res = $stmt->get_result();
$rowClientCountOfBdm = $res->num_rows;
array_push($bdeAndClientCount,$rowClientCountOfBdm);
$stmt_bde = $this->connection->prepare("select * from users where user_manager_id = ?");
$stmt_bde->bind_param("i", $bdm_emp_id);
$stmt_bde->execute();
$res_bde = $stmt_bde->get_result();
$rowBdesCountOfBdm = $res_bde->num_rows;
array_push($bdeAndClientCount,$rowBdesCountOfBdm);
$stmt->close();
return $bdeAndClientCount;
}

public function getClientContactById($contactId) {
    $stmt = $this->connection->prepare("select * from client_contacts where client_contact_id = ?");
    $stmt->bind_param("i", $contactId);
    $stmt->execute();
    $res = $stmt->get_result();
...
public function getClientContactsByCompanyId($companyId){
    $stmt = $this->connection->prepare("select * from client_contacts where client_company_id = ? order by client_contact_id desc");
    $stmt->bind_param("i", $companyId);
    $stmt->execute();
    $res = $stmt->get_result();
...
public function addClientContact($contact){
 ...
 public function addclientwithcompany($company,$contact){
        ...
        $query = $this->connection->query("select max(client_company_id) from client_companies");
        $max_client_company_id = $query->fetch_assoc()['max(client_company_id)'];
        $max_client_company_id = $max_client_company_id + 1;
        $this->connection->query("unlock tables"); 
       // $max_id = $max_client_company_id->get_result(); 
        //inserting client
        $firstName = $contact->getFirstName();
        $lastName = $contact->getLastName();
        $email = $contact->getEmail();   
        ...
        $max_client_company_id = $row_company->client_company_id;
        }
        $userId = $_SESSION['userId'];
        $emailTimeout = 24; 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	
        $stmt = $this->connection->prepare("insert into client_contacts (client_contact_first_name, client_contact_last_name, client_contact_email, client_contact_category, client_contact_designation,client_contact_phoneno1, client_contact_phoneno2, city_id, state_id, country_id, client_contact_address, client_contact_linkedin, client_contact_facebook, client_contact_twitter, client_contact_status, client_contact_added, client_company_id, assoc_manager_id, assoc_user_id, email_timeout) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssiiiiissssssiiis", $firstName, $lastName, $email, $category, $designation, $phone1,$phone2, $city, $state, $country, $address, $linkedIn, $facebook, $twitter, $status, $added, $max_client_company_id, $assocManager, $assocUser, $emailTimeout);
        $stmt->execute();
        $stmt->close();  
    }
...
public function fetchclientdata($userId,$offset){
    ...
    public function getCountOfCompanyAndClientCounts($user_Id){
        $stmt_bde = $this->connection->prepare("select user_emp_id from users where user_id = ?");
        $stmt_bde->bind_param("i", $user_Id);
        $stmt_bde->execute();
        $res_bde = $stmt_bde->get_result();
        $row_bde = $res_bde->fetch_assoc();
        $bde_emp_id = $row_bde['user_emp_id'];
        $ComapnyAndClientCount = array(); 
        $stmt = $this->connection->prepare("select * from client_contacts where assoc_user_id = ?");
        $stmt->bind_param("i", $bde_emp_id);
        $stmt->execute();
        $res = $stmt->get_result();
        $rowClientCountOfBde = $res->num_rows;
        array_push($ComapnyAndClientCount,$rowClientCountOfBde);
        $stmt_company = $this->connection->prepare("select * from client_companies");
        $stmt_company->execute();
        $res_company = $stmt_company->get_result();
        $rowCompanyCountofBde = $res_company->num_rows;
        array_push($ComapnyAndClientCount,$rowCompanyCountofBde);
        $stmt->close();
        return $ComapnyAndClientCount;
    }
